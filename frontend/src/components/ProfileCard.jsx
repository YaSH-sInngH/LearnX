    ...
    <div className="card overflow-hidden relative">
      ...
      {isOwnProfile && !isEditing && (
        <button
          ...
          className="absolute top-4 right-4 btn btn-primary px-4 py-2 shadow-large hover:shadow-glow transition-all duration-200 z-10"
        >
      ...
      {/* Header with Avatar */}
      <div className="relative bg-gradient-to-br from-primary-500 via-primary-600 to-primary-700 p-8 flex flex-col items-center">
        <div className="relative mb-4">
          <img
            src={avatarUrl}
            alt={profile?.name || 'User'}
            className="w-24 h-24 rounded-full object-cover border-4 border-white/20 shadow-large ring-4 ring-white/30 transition-transform duration-200 hover:scale-105"
            onError={e => { e.target.src = '/default-avatar.png'; }}
          />
          {isOwnProfile && isEditing && (
            <label className="absolute bottom-0 right-0 bg-white dark:bg-secondary-800 rounded-full p-2 shadow-large cursor-pointer hover:shadow-glow transition-all duration-200 border-2 border-white dark:border-secondary-700">
              <input
                type="file"
                accept="image/*"
        ...
        <h2 className="text-2xl font-bold text-white mb-2 font-display">{profile?.name}</h2>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {['JavaScript', 'Python', 'React', 'Node.js', 'Data Science', 'Machine Learning', 'Web Design', 'Mobile Development'].map((skill) => (
                  <label key={skill} className="flex items-center bg-primary-50 dark:bg-primary-900/30 px-3 py-2 rounded-lg cursor-pointer text-primary-700 dark:text-primary-300 font-medium text-sm hover:bg-primary-100 dark:hover:bg-primary-900/50 transition-colors">
                    <input
                      type="checkbox"
            ...
            {profile?.role === 'Learner' && (
              <div className="grid grid-cols-2 gap-4 pt-4 border-t border-secondary-200 dark:border-secondary-700">
                <div className="text-center">
                  ...
                  <div className="text-xs text-secondary-500 dark:text-secondary-400">Experience Points</div>
                </div>
                <div className="text-center">
